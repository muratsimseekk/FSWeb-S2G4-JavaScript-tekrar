...
function CemberinAlani(yaricap2, piDegeri) {
  const alan = () => piDegeri * Math.pow(yaricap2, 2);
...
const ucebolunenlerintoplami = ucetambolunenler.reduce(
  (acc, value) => acc + value,
  0
);

console.log("Üçe bölünenlerin toplamı : ", ucebolunenlerintoplami);
...
// sayilar.forEach((sayilarKucuk) => {
//   if (sayilarKucuk < 500) {
//     besyuzdenkucuksayilar.push(sayilarKucuk);
//   }
// });
...
for (let eleman in sayiObj) {
  if (sayiObj[eleman] > 1) {
    tekraredensayilar.push(
      `${eleman} sayısı ${sayiObj[eleman]} kere tekrar edilmiştir`
    );
  }
}
